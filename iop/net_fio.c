...
static inline int pko_lwip_send(int sock, void *buf, int len, int flag)
{
	int ret;
...
int pko_accept_pkt(int sock, char *buf, int len, int pkt_type)
{
	...
	length = pko_recv_bytes(sock, buf + sizeof(pko_pkt_hdr), hlen - sizeof(pko_pkt_hdr));
...
int pko_open_file(char *path, int flags)
{
	...
	if (!pko_accept_pkt(pko_fileio_sock, recv_packet, sizeof(pko_pkt_file_rly), PKO_OPEN_RLY))
	{
...
int pko_close_file(int fd)
{
	...
	if (!pko_accept_pkt(pko_fileio_sock, (char *)closerly, sizeof(pko_pkt_file_rly), PKO_CLOSE_RLY))
	{
...
int pko_lseek_file(int fd, unsigned int offset, int whence)
{
	...
	if (!pko_accept_pkt(pko_fileio_sock, (char *)lseekrly, sizeof(pko_pkt_file_rly), PKO_LSEEK_RLY))
	{
...
int pko_write_file(int fd, char *buf, int length)
{
	...
	while (writtenbytes < length)
	{
		...
		memcpy(&send_packet[sizeof(pko_pkt_write_req)], &buf[writtenbytes], nbytes);
		...
		// Get reply
		if (!pko_accept_pkt(pko_fileio_sock, (char *)writerly, sizeof(pko_pkt_file_rly), PKO_WRITE_RLY))
		{
...
int pko_read_file(int fd, char *buf, int length)
{
	...
	if (!pko_accept_pkt(pko_fileio_sock, (char *)readrly, sizeof(pko_pkt_read_rly), PKO_READ_RLY))
	{
...
int pko_remove(char *name)
{
	...
	if (!pko_accept_pkt(pko_fileio_sock, recv_packet, sizeof(pko_pkt_file_rly), PKO_REMOVE_RLY))
	{
...
int pko_mkdir(char *name, int mode)
{
	...
	if (!pko_accept_pkt(pko_fileio_sock, recv_packet, sizeof(pko_pkt_file_rly), PKO_MKDIR_RLY))
	{
...
int pko_rmdir(char *name)
{
	...
	if (!pko_accept_pkt(pko_fileio_sock, recv_packet, sizeof(pko_pkt_file_rly), PKO_RMDIR_RLY))
	{
...
int pko_open_dir(char *path)
{
	...
	if (!pko_accept_pkt(pko_fileio_sock, recv_packet, sizeof(pko_pkt_file_rly), PKO_OPENDIR_RLY))
	{
...
int pko_read_dir(int fd, void *buf)
{
	...
	if (!pko_accept_pkt(pko_fileio_sock, recv_packet, sizeof(pko_pkt_dread_rly), PKO_READDIR_RLY))
	{
...
int pko_close_dir(int fd)
{
	...
	if (!pko_accept_pkt(pko_fileio_sock, (char *)closerly, sizeof(pko_pkt_file_rly), PKO_CLOSEDIR_RLY))
	{
