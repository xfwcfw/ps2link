...
static int pkoGSExec(pko_pkt_gsexec_req *cmd)
{
	int fd;
	int len;
	...
	asm __volatile__("move   $10, %0;"
			 "move   $11, %1;"
			 "lui    $8, 0x1001;"
			 "sw     $11, -24544($8);"
			 "sw     $10, -24560($8);"
			 "ori    $9, $0, 0x101;"
			 "sw     $9, -24576($8);" ::"r"(dataBuffer),
			 "r"((ntohs(cmd->size)) / 16));

	// dmawait for GIF channel
	asm __volatile__("lui    $8, 0x1001;"
			 "dmawait:"
			 "lw     $9, -24576($8);"
			 "nop;"
			 "andi   $9, $9, 0x100;"
...
static int pkoDumpMem(pko_pkt_mem_io *cmd)
{
	int fd;
	int total, len;
	unsigned int offset;
...
static int pkoWriteMem(pko_pkt_mem_io *cmd)
{
	int fd, len, total;
	unsigned int offset;
	unsigned int size;
...
static int pkoDumpReg(pko_pkt_dump_regs *cmd)
{
	int fd, ret = 0;
	char path[PKO_MAX_PATH];
	unsigned int i, j, size;
	unsigned int dmaregs[52] = {
		0x1000e000, 0x1000e010, 0x1000e020, 0x1000e030, 0x1000e040, 0x1000e050, 0x1000e060, 0x1000f520, // Dma common registers
	...
	unsigned int intcregs[2] = { 0x1000F000, 0x1000F010 };
	unsigned int gsregs[2] = { 0x12001000, 0x12001080 };
	unsigned int gifregs[8] = { 0x10003020, 0x10003040, 0x10003050, 0x10003060, 0x10003070, 0x10003080, 0x10003090, 0x100030a0 };
	unsigned int timerregs[14] = { 0x10000000, 0x10000010, 0x10000020, 0x10000030, 0x10000800, 0x10000810, 0x10000820,
				       0x10000830, 0x10001000, 0x10001010, 0x10001020, 0x10001800, 0x10001810, 0x10001820 };
	unsigned int sifregs[1] = { 0x1000F230 };
	unsigned int vif0regs[19] = { 0x10003800, 0x10003810, 0x10003820, 0x10003830, 0x10003840, 0x10003850, 0x10003860, 0x10003870, 0x10003880, 0x10003890,
				      0x100038d0, 0x10003900, 0x10003910, 0x10003920, 0x10003930, 0x10003940, 0x10003950, 0x10003960, 0x10003970 };
	unsigned int vif1regs[23] = { 0x10003c00, 0x10003c10, 0x10003c20, 0x10003c30, 0x10003c40, 0x10003c50, 0x10003c60, 0x10003c70,
				      0x10003c80, 0x10003c90, 0x10003ca0, 0x10003cb0, 0x10003cc0, 0x10003cd0, 0x10003ce0, 0x10003d00,
				      0x10003d10, 0x10003d20, 0x10003d30, 0x10003d40, 0x10003d50, 0x10003d60, 0x10003d70 };
	unsigned int fiforegs[2] = { 0x10005000, 0x10007000 };
	unsigned int ipuregs[4] = { 0x10002000, 0x10002010, 0x10002020, 0x10002030 };
	unsigned int REGALL_SIZE = sizeof(dmaregs) + sizeof(intcregs) + sizeof(timerregs) + sizeof(gsregs) + sizeof(sifregs) + sizeof(fiforegs) +
				   sizeof(gifregs) + sizeof(vif0regs) + sizeof(vif1regs) + sizeof(ipuregs);
...
static int pkoStopVU(pko_pkt_stop_vu *cmd)
{
	if (ntohs(cmd->vpu) == 0)
	{
		asm("ori     $25, $0, 0x1;"
...
static int pkoStartVU(pko_pkt_start_vu *cmd)
{
	if (ntohs(cmd->vpu) == 0)
	{
		asm("ori     $25, $0, 0x2;"
...
static int cmdThread()
{
	...
	while (!done)
	{
		...
		case PKO_IOPEXCEP_CMD:
			iopException(sifDmaDataPtr[3], sifDmaDataPtr[4], sifDmaDataPtr[5], sifDmaDataPtr[2], &sifDmaDataPtr[6], sifDmaDataPtr[41],
				     (char *)&sifDmaDataPtr[43]);
			break;
		default:
